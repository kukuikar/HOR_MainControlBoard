...
if(millis() - tmr_oled > 50)
  {
  tmr_oled = millis();
  if(digitalRead(MANUAL_LIFT_PIN) == LOW && pointer < 7)
  {
    ...
    switch(ints[0])
    {
      ...
      //if(millis() - tmr > 50)
      {
        ...
        if(servoNum == 6)
        {
          ...
          {       
            val = (val > 90 && digitalRead(LIFT_LIMIT_PINS[0][i]) == LOW) || (val < 90 && digitalRead(LIFT_LIMIT_PINS[1][i]) == LOW) ? val : 90;     
            //lifts[i].drive(val);
          }
        }
        else
        {
          val = (val > 90 && digitalRead(LIFT_LIMIT_PINS[0][servoNum]) == LOW) || (val < 90 && digitalRead(LIFT_LIMIT_PINS[1][servoNum]) == LOW) ? val : 90;
          //lifts[servoNum].drive(val);
        }      
      break;
      }
    }
